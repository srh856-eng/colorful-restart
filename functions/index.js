...
// Hard-coded mapping ensuring VSBKIKB static indexing layout across all team paths
const PUZZLE_DATA = {
  CW: { heading: "Quest CW", description: "Solve the crossword clues to find your path forward.", prompt: "Did you solve the crossword? What is the unscrambled word?", answer: "BEAUTIFUL", letterPayload: "BI", targetIndices: [3, 4] },
  BP: { heading: "Quest BP", description: "Are you good at additions? Find the marked locations — but watch out for decoys.", prompt: "What is the total of the numbers you found?", answer: "5698", letterPayload: "K", targetIndices: [5] },
  MR: { heading: "Quest MR", description: "Pablo the penguin is missing. Find where every other office member was, and Pablo will be in the last remaining cell. Pablo left a passcode hidden in that room.", prompt: "Enter Pablo's passcode", answer: "PABLOLOVESYOU", letterPayload: "VS", targetIndices: [0, 1] },
  LR: { heading: "Quest LR", description: "Sam is the new intern at the Protective department. He walked to HR, then was sent to IT. His route traced a letter — can you find it?", prompt: "What letter did Sam's route trace?", answer: "L", letterPayload: "B", targetIndices: [2] },
  FR: { heading: "Quest FR", description: "Head to the pantries and read the facts. Some have something odd hidden inside — be careful, not all of them do.", prompt: "Unscramble the odd letters you found", answer: "EARTH", letterPayload: "K", targetIndices: [6] }
};
...
const HINTS = {
  ...
  FINALE:   "Use the Vigenère cipher wheel tool. Align the outer ring letter with your inner key character to decipher parameters."
};
...
exports.submitStageAnswer = onRequest((req, res) => {
  return cors(req, res, async () => {
    try {
      ...
      if (state.isCompleted) return res.status(209).json({ error: { message: "Hunt completed." } });
      ...
      let lettersArr = (state.letters || "_______").split("");
      while(lettersArr.length < 7) lettersArr.push("_");

      for (let i = 0; i < puzzle.targetIndices.length; i++) {
        const targetIdx = puzzle.targetIndices[i];
        lettersArr[targetIdx] = puzzle.letterPayload[i];
      }
      
      const updatedLetters = lettersArr.join("");
      ...
      return res.status(200).json({ 
        result: { 
          correct: true, 
          nextStage, 
          animationType: ANIMATIONS[puzzle.key] || "Great",
          newLetters: updatedLetters,
          justDropped: puzzle.letterPayload
        } 
      });
    } catch (err) {
      return res.status(500).json({ error: { message: err.message } });
...
exports.requestHint = onRequest((req, res) => {
  return cors(req, res, async () => {
    try {
      ...
      if (!state) return res.status(404).json({ error: { message: "State data missing." } });
      if ((state.hintsUsed || 0) >= MAX_HINTS) {
      ...
      await teamRef.update({ hintsUsed: newHintCount, lastActive: Date.now() });

      return res.status(200).json({ 
        result: { 
          success: true, 
          hint: hintText, 
          remaining: Math.max(0, MAX_HINTS - newHintCount) 
        } 
      });
    } catch (err) {
      return res.status(500).json({ error: { message: err.message } });
...
exports.submitFinale = onRequest((req, res) => {
  return cors(req, res, async () => {
    try {
      ...
      // Fix 1: Accept 'CENTURY' as the targeted final verification value
